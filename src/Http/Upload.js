    ...
    return function multerMiddleware(req, res, next) {
        ...
        function abortWithError(uploadError) {
            ...
            pendingWrites.onceZero(function() {
                ...
                removeUploadedFiles(uploadedFiles, remove,
        ...
        busboy.on('field',
            function(fieldname, value, fieldnameTruncated, valueTruncated) {
                ...
                if (valueTruncated) return abortWithCode('LIMIT_FIELD_VALUE',
                    fieldname);

                // Work around bug in Busboy (https://github.com/mscdex/busboy/issues/6)
                if (limits && limits.hasOwnProperty('fieldNameSize')) {
                    if (fieldname.length >
                        limits.fieldNameSize) return abortWithCode(
                        'LIMIT_FIELD_KEY');
                }
        ...
        busboy.on('partsLimit',
            function() { abortWithCode('LIMIT_PART_COUNT'); });
        busboy.on('filesLimit',
            function() { abortWithCode('LIMIT_FILE_COUNT'); });
        busboy.on('fieldsLimit',
            function() { abortWithCode('LIMIT_FIELD_COUNT'); });
